'''
Escenario
La tarea es preparar un código simple para evaluar o encontrar el tiempo final 
de un periodo de tiempo dado, expresándolo en horas y minutos. 
La hora de inicio se da como un par de horas (0..23) y minutos (0..59). 
El resultado debe ser mostrado en la consola.

Por ejemplo, si el evento comienza a las 12:17 y dura 59 minutos, terminará a las 13:16.

No te preocupes si tu código no es perfecto - está bien si acepta una hora invalida 
- lo más importante es que el código produzca una salida correcta acorde a la entrada dada.

Prueba el código cuidadosamente. Pista: utilizar el operador % puede ser clave para el 
éxito.

'''
hora_inicio = int(input( "¿Cual es la hora de inicio? "))
min_inicio = int(input( "¿Minutos de inicio? "))
tiempo_dur = int(input( "¿Cuanto dura el evento? "))

min_total = min_inicio + tiempo_dur
total_min = min_total % 60

horas_extra = min_total // 60
horas_totales =  (horas_extra + hora_inicio) % 24
print(horas_totales, ":",total_min)
